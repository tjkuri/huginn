...
- **Split projections use a two-step overall-baseline approach.** Split rates (vs-LHP/vs-RHP for batters, vs-LHB/vs-RHB for pitchers) are anchored to the overall Marcel rather than computed fully independently. Step 1: compute overall Marcel projection. Step 2: for each split bucket, Marcel-blend per-season ratios (`split_rate[stat] / overall_rate[stat]`) toward the **league-wide platoon ratio for that handedness bucket** (`LEAGUE_AVERAGES[bucket][stat] / overall_hand_rate[stat]`, where `overall_hand_rate` is the equal-weight mean of the two matchups that share the player's hand). Step 3: apply `projected_split_rate = overall_rate * blended_ratio`. A player with rich split history gets a ratio close to their observed platoon split; thin history regresses toward the league-wide platoon ratio (not 1.0) — so the projection expresses the league-average platoon effect, not "no platoon effect." The split-ratio blend inherits the same year-weight schedule as the overall blend: 5/4/3 for batters, 3/2/1 for pitchers. Ratio regression constants are hand-specific (PA equivalents, applied to every rate stat): LHB = 500, RHB = 1100 (RHB splits regress more heavily because they stabilize more slowly), switch hitters = 600. For switch hitters the numerator comes from the matchup that matches their effective batting side ((R,L) for vs_lhp, (L,R) for vs_rhp) with `overall_switch` (mean of (R,L) and (L,R)) as denominator. Pitchers use a single ratio constant of 1000 BF for all stats; pitcher targets use `overall_lhp` (mean of (L,L) and (R,L)) or `overall_rhp` (mean of (L,R) and (R,R)) as the denominator.
